...
function exe()
{
	...
	*/
	...
	$targets = array(	'from:AccessMWC',
						...
						'#MobileWish');
	*/
	$targets = array('#suzuD','#suzuD_map');
	
	// 質問用のハッシュ「#suzuD1」～「#suzuD100」をすべて登録
	for($num=1;$num<=47;$num++)
	{
		$q_hash = "#suzuD".$num;
		array_push($targets,$q_hash);
	}
	...
	/*
$json = $result;//$consumer->sendRequest($req)->getBody();
	$res = json_decode($json);
	
	
	
	// 2011.6.26. added by mio 
	// 第2引数を true　にすると、連想配列でjsonを取得できる。
	...
	// 2011.6.26. added by mio 
	// これで Geoが取得できた。
	echo "geo".$jsonArray["results"][0]["geo"]["coordinates"][0]."<br>";
*/
...
function  echoTweetData( $value )
{
	...
	echo $value["text"]."<br />";
	echo $value["geo"]["coordinates"][0]."<br />";
	echo $value["geo"]["coordinates"][1]."<br />";
	echo "<br />";
}

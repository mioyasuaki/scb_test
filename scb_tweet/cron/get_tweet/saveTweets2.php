...
TwitterAPIからiplusd関係のツイートを取得してDBへ登録
...
function exe()
{
	...
	*/
	...
	{
		$req .= $conditions[$i];
		if($i != sizeof($conditions)-1) $req .= "+OR+";
	}
	*/
	$noConditions = true;
	for($i = 0; $i < sizeof($conditions); $i++)
	...
	// 返信先がないか調査(失敗・・・) --------------------------------------
	// in_reply_to_status_id、in_reply_to_user_idがsearchAPIから取得できなかった代替として入れようかと思ったが、途中で断念。
	
	/*
	foreach( $result as $value )
	{
		$tweet_id = $value->id;
		$res = reqAPI("http://api.twitter.com/1/statuses/retweets/$tweet_id.json");
		if($res!=null) 
		{
			echo "-------------------------------<br />";
			echo "$tweet_id<br />";
